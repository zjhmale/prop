...
public:
  ...
  ~GCBitMap();
   ...
   //  Selectors
   ///////////////////////////////////////////////////////////////////////////
   inline void * lowest()  const { return lowest_addr; }
   inline void * highest() const { return highest_addr; }
   ...
   ///////////////////////////////////////////////////////////////////////////
   inline void * bottom() const { return bitmap_bot; }
   inline void * top()    const { return bitmap_top; }
   ...
   {  if (start < lowest_addr)  start = lowest_addr;
      if (stop  > highest_addr) stop = highest_addr;
      register Glob * block = 
         bitmap + ((size_t)start/ (GC_ALIGNMENT * 8 * sizeof(Glob)));
      register size_t len   = 
         ((Byte*)stop - (Byte*)start) / (GC_ALIGNMENT * 8 * sizeof(Glob));
      while (len > 0) { len--; *block++ = 0; }
   }
