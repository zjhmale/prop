...
template <class K, class V, class C>
   ...
   {  /////////////////////////////////////////////////////////////////////
      ...
      for (count = table_size ;; count--) {
         switch (status[i]) {
            case Cell_deleted: if (count > 0) break;
            case Cell_unused:  
               elem_count++;
            ...
            case Cell_used:    
               int r = C::compare(keys[i],key);
               if (r == 0) { values[i] = value; return (Ix)(i+1); }
               if (r > 0) { // exchange entries and continue
                  K k = key; key = keys[i]; keys[i] = k;
                  V v = value; value = values[i]; values[i] = v;
                  h = C::hash(key); inc = 0;
               }
         }
         if (inc == 0) { 
...
template <class K, class V, class C>
   ...
   {  if (new_size <= elem_count) 
      ...
      //////////////////////////////////////////////////////////////////
      if (new_size <= table_size) new_size = table_size + 32;
      ...
      for (int i = 0; i < table_size; i++) {
         if (status[i] == Cell_used) {
            ...
            for (;;) {
               ...
               if (r > 0) { // exchange entries and continue
                  K k = new_keys[j]; new_keys[j] = keys[i]; keys[i] = k;
                  V v = new_values[j]; new_values[j] = values[i]; values[i] = v;
                  h = C::hash(k); inc = 0;
               }
               if (inc == 0) { 
...
//////////////////////////////////////////////////////////////////////////
template <class K, class V, class C>
   Bool OHashTable2<K,V,C>::remove(const K& key)
   {  Ix i;
      ///////////////////////////////////////////////////////////////////
