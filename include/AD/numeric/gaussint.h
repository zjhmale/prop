...
struct GaussInt {
   ...
   // In place arithmetic  
   /////////////////////////////////////////////////////////////////////
   void operator += (const GaussInt& c) { real += c.real; imag += c.imag; }
   void operator -= (const GaussInt& c) { real -= c.real; imag -= c.imag; }
   void operator *= (const GaussInt& c) { *this = *this * c; }
   void operator /= (const GaussInt& c) { *this = *this / c; }
   void operator += (int r)             { real += r; }
   void operator -= (int r)             { real -= r; }
   void operator *= (int r)             { real *= r; imag *= r; }
   void operator /= (int r)             { real /= r; imag /= r; }
   ...
   //////////////////////////////////////////////////////////////////////
   GaussInt operator ! () const { return GaussInt(real,-imag); }
   friend int norm(const GaussInt& c) 
      { return c.real * c.real + c.imag * c.imag; }
   friend double abs(const GaussInt& c) { return sqrt(norm(c)); }

   ////////////////////////////////////////////////////////////////
   // Input and output
   ////////////////////////////////////////////////////////////////
   friend ostream& operator << (ostream&, const GaussInt&);
   friend istream& operator >> (istream&, GaussInt&);
};
