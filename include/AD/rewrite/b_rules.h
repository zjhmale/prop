...
public:
  ...
  ~BURS_RuleSet ();
   ...
   //  Selectors.
   ///////////////////////////////////////////////////////////////////////////
   inline int number_of_reductions       ()      const { return reduction_count; }
   inline int number_of_leaf_reductions  ()      const { return leaf_reduction_count; }
   inline int number_of_chain_rules      ()      const { return chain_rule_count; }
   inline int number_of_non_terminals    ()      const { return non_terminal_count; }
   inline const LeafReduction& leaf      (int i) const { return leaf_reduction_table[i]; }
   inline const Reduction&     reduction (int i) const { return *reduction_table[i]; }
   inline const ChainRule&     chain_rule(int i) const { return chain_rule_table[i]; }
   inline const ReductionList * reductions_of(Functor f) const { return reductions[f]; }
   inline Bool  contains_wild_card() const             { return has_wild_card; }
