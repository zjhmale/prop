...
{  Matrix r(M,N);
   for (int i = len - 1; i >= 0; i--) r.array[i] = -array[i]; return r;
}
...
{  Matrix r(m.M,m.N);
   for (int i = m.len - 1; i >= 0; i--) r.array[i] = a + m.array[i]; 
   return r;
}
Matrix operator + (const Matrix& m, double a)
{  Matrix r(m.M,m.N);
   for (int i = m.len - 1; i >= 0; i--) r.array[i] = m.array[i] + a; 
   return r;
}
Matrix operator - (double a, const Matrix& m)
{  Matrix r(m.M,m.N);
...
{  for (int i = len - 1; i >= 0; i--) array[i] = -array[i]; return *this; }
Matrix& Matrix::operator += (double a)
{  for (int i = len - 1; i >= 0; i--) array[i] += a; return *this; }
Matrix& Matrix::operator -= (double a)
{  for (int i = len - 1; i >= 0; i--) array[i] -= a; return *this; }
Matrix& Matrix::operator *= (double a)
{  for (int i = len - 1; i >= 0; i--) array[i] *= a; return *this; }
...
{  a.can_add(b);
   Matrix r(a.M,a.N);
   for (int i = a.len - 1; i >= 0; i--) r.array[i] = a.array[i] + b.array[i]; 
   return r;
}
...
{  can_add(m);
   for (int i = len - 1; i >= 0; i--) array[i] += m.array[i]; 
   return *this; 
}

Matrix& Matrix::operator -= (const Matrix& m)
{  can_add(m);
   for (int i = len - 1; i >= 0; i--) array[i] -= m.array[i]; return *this; 
}
