   ...
   for (;;) {
      ...
      foreach_page (p) {
         if (page_table[p] == from_space && gc_table[p] == 0) {
            if (found == 0) first_page = p;
            found++;
         } else {
            if (found >= page_count) break; // found!
            found = 0;
         }
      }
      ...
      if (found >= page_count) {
         ...
         if (growth < bytes_need) growth = bytes_need;
         if (allocate_new_pages(gc, growth) == 0) return 0;
      }
   }
}
...
{  // First, round it up to the size of a page
   ...
   if (object_map.bottom() < low)    low = object_map.bottom();
   if (live_map.bottom() < low)      low = live_map.bottom();
   if (object_map.top() > high)      high = object_map.top();
   if (live_map.top() > high)        high = live_map.top();
#ifdef GC_HAS_CROSS_HEAP_POINTERS
   if (traversed_map.bottom() < low) low = traversed_map.bottom();
   if (traversed_map.top() > high)   high = traversed_map.top();
#endif 
...
{ 
   ...
   if (bottom < lowest_tracked_addr || lowest_tracked_addr == 0) {
      new_low = bottom; expansion = true;
   } else {
      new_low = lowest_tracked_addr;
   }
   if (top > highest_tracked_addr) {
      new_high = top; expansion = true;
   } else {
...
{  register GC_ID id = gc->gc_id();
   for ( ;number_of_pages > 0; number_of_pages--, page_id++) {
      if (gc_table[page_id] != id || page_table[page_id] <= blacklisted) {
         cerr << "[ Bug: failed to deallocate page " << page_id << " ]\n";
      } else {
         gc_table   [page_id] = 0;
         page_table [page_id] = from_space;
...
{  GCPageId p = GC_PAGE_ID(start);
   ...
   if (p < LOWEST_PAGE())  p = LOWEST_PAGE();
   if (q > HIGHEST_PAGE()) q = HIGHEST_PAGE();
   for ( ;p < q; p++) page_table[p] = status;
}
...
{  void * start = GC_ROUNDUP_PAGE_ADDR(addr);
   ...
   if (p < LOWEST_TRACKED_PAGE())  p = LOWEST_TRACKED_PAGE();
   if (q > HIGHEST_TRACKED_PAGE()) q = HIGHEST_TRACKED_PAGE();
   for ( ; p < q; p++) 
...
#ifdef DEBUG_GC
      if (p < q)
         cerr << "[ Blacklisting system heap addresses " << (void*)GC_PAGE_ADDR(p) 
              << " ... " << (void*)GC_PAGE_ADDR(q) << " ]\n";
#endif
      for ( ; p < q; p++) {
         if (page_table[p] != not_allocated) {
            cerr << "[ Bug: trying to blacklist allocated addresses "
                 << (void*)GC_PAGE_ADDR(p) << " ... " 
...
#ifdef DEBUG_GC
      if (p < q)
         cerr << "[ Unblacklisting system heap addresses " << (void*)GC_PAGE_ADDR(p)
              << " ... " << (void*)GC_PAGE_ADDR(q) << " ]\n";
#endif
      for ( ; p < q; p++) {
         if (page_table[p] != blacklisted) {
            cerr << "[ Bug: trying to unblacklist allocated addresses "
                 << (void*)GC_PAGE_ADDR(p) << " ... " 
                 << (void*)GC_PAGE_ADDR(p+1) << " ]\n";
         } else {
            page_table[p] = not_allocated;
...
{  
   ...
   for (start = data_bottom; start < data_top; start = stop) {
      while (i < n) {
         ...
         { start = (void**)B[i].stop; i++; } 
         else break;
      }
      if (i < n) stop = (void**)B[i].start;
      else       stop = data_top;
      if (start < stop) {
...
//////////////////////////////////////////////////////////////////////////////
//  Error reporting.  
//  We'll just print a message to cerr.
//////////////////////////////////////////////////////////////////////////////
ostream& HM::error (const char * message) 
{  return cerr << "HM: " << message << '\n';
}
...
//  We'll just print a message to cerr.
//////////////////////////////////////////////////////////////////////////////
ostream& HM::print_report (ostream& log) 
