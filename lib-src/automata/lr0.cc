...
{   
   ...
   foreach_item (i, s) {
      ...
      if (X > Grammar::END_PRODUCTION) { // terminals and non-terminals only
         register ItemSet& set = *s0[X];
         //
         // the shifted item (on symbol X) must be $i+1$ according to 
         // our item numbering scheme.
         //
         if (is_used[X] != &s) set.clear(); // clear it first
         ...
         // 
         if (G.isNonTerminal(A) && ! set[non_term_to_one_item[A]]) 
...
{  
   ...
   {  LRItem item;
      item.pos = 0;
      for (int j = max_non_term; j >= 0; j--) non_term_to_item[j] = 0;
      for (int i = 0; i < G.size(); i++) {
         item.P        = G[i]; 
   ...
   {  for (State s = 0; s < state_count; s++) {
         register const ItemSet& set = *all_item_sets[s];
         ItemNum i; 
