...
void LexerBuffer::init()
{
   ...
   anchored = true; current_context = 0; saved_char = -1;
   pinned = false; more_input = false;
}
...
LexerBuffer:: LexerBuffer(char * s)  
   { init(); pinned = true; 
     cursor = cursor_limit = buffer = s; buffer_limit = s + strlen(s); 
   }
LexerBuffer:: LexerBuffer(char * s, size_t l)
   { init(); pinned = true; 
     cursor = cursor_limit = buffer = s; buffer_limit = s + l; 
   }
LexerBuffer::~LexerBuffer() 
   { reset(); }
...
//////////////////////////////////////////////////////////////////////////////
void LexerBuffer::reset() { if (! pinned) delete [] buffer; init(); }
...
{  pinned = true; 
   cursor = cursor_limit = buffer = s; buffer_limit = s + len;
}

void LexerBuffer::set_buffer(char * buf) { set_buffer(buf,strlen(buf)); }
