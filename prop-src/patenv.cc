...
//  Destructor
///////////////////////////////////////////////////////////////////////////
PatternVarEnv::~PatternVarEnv () { delete impl; }
...
{  register const BINDING * top   = &impl->env.top();
   ...
   for ( ;top >= limit; top--)
      if (top->var == v) {
         if (from_current) {  // check whether it is from the current scope
...
///////////////////////////////////////////////////////////////////////////
Bool PatternVarEnv::separate_guard() { return impl->separate_guards.top(); }
Bool PatternVarEnv::tree_grammar()   { return impl->tree_grammar.top(); }
Exp  PatternVarEnv::guard()          { return impl->guards.top(); }
Exp  PatternVarEnv::guard(Exp e)     
{  Exp e2 = guard();
   if (e == NOexp) return e2;
   if (e2 == NOexp) return e;
   return BINOPexp("&&",e2,e);
}
void PatternVarEnv::add_guard(Exp e)
{  if (e != NOexp) 
   {  Exp& g = impl->guards.top();
      if (g != NOexp) g = BINOPexp("&&",g,e);
      else g = e;
      if (options.debug) msg("%Lnew guard expression: %e\n", g);
   }
}
