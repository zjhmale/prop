...
void   MEM::use_global_pools() { current_pool = &global_pool; globals++; }
void   MEM::use_local_pools()  { if (--globals == 0) current_pool = &mem_pool;}
void * MEM::operator new(size_t n) { return (*current_pool)[n]; }
...
//
///////////////////////////////////////////////////////////////////////////////
Loc::Loc() { begin_line = first_line; end_line = line; file_name = file;
             first_line = line; }
Loc::Loc(int a, int b) 
           { begin_line = a; end_line = b; file_name = file;
             first_line = line; }

void Loc::set_loc() const { line = begin_line; file = file_name; }
...
{  // Locate the . in the file name and
   ...
   if (slash == 0) slash = input_file_name-1;
...
static void emit_headers(Compiler& C)
{
   ...
   /////////////////////////////////////////////////////////////////////////
   Bool inc = false;
   if (Used::rewriting)  { C.pr ("#define PROP_REWRITING_USED\n"); inc = true; }
   if (Used::infer)      { C.pr ("#define PROP_INFERENCE_USED\n"); inc = true; }
   if (Used::gc)         { C.pr ("#define PROP_GARBAGE_COLLECTION_USED\n"); inc = true; }
   if (Used::refcount)   { C.pr ("#define PROP_REFCOUNT_USED\n"); inc = true; }
   if (Used::printer)    { C.pr ("#define PROP_PRINTER_USED\n"); inc = true; }
   if (Used::persistence) { C.pr ("#define PROP_PERSISTENCE_USED\n"); inc = true; }
   if (Used::objc)       { C.pr ("#define PROP_OBJC_USED\n"); inc = true; }
   if (Used::regexp)     { C.pr ("#define PROP_REGEXP_MATCHING_USED\n"); inc = true; }
   if (Used::string_match) { C.pr ("#define PROP_STRCMP_USED\n"); inc = true; }
   if (Used::equality)   { C.pr ("#define PROP_EQUALITY_USED\n"); inc = true; }
   if (Used::unification) { C.pr ("#define PROP_UNIFICATION_USED\n"); inc = true; }
   if (Used::vector)     { C.pr ("#define PROP_VECTOR_USED\n"); inc = true; }
   if (Used::parser)     { C.pr ("#define PROP_PARSER_USED\n"); inc = true; }
   if (Used::quark)      { C.pr ("#define PROP_QUARK_USED\n"); inc = true; }
   if (Used::bigint)     { C.pr ("#define PROP_BIGINT_USED\n"); inc = true; }
   if (Used::graph_type) { C.pr ("#define PROP_GRAPHTYPE_USED\n"); inc = true; }
   if (options.trace)    { C.pr ("#define PROP_TRACE_ON\n"); inc = true; }
   ...
   if (inc) C.pr ("#include <propdefs.h>\n");
}
...
int process_input(PropOptions& options)
{
   ...
   //  Open output file
   ////////////////////////////////////////////////////////////////////////////
   if (errors > 0) options.emit_code = false;
   ...
   {  if (options.to_stdout) {
      ...
      //  Cleanup
      /////////////////////////////////////////////////////////////////////////
      if (clean_up_file) delete output_stream;
